export const site = {
  ...
  social: {
    facebookMarketplace:
      process.env.NEXT_PUBLIC_FACEBOOK_MARKETPLACE_URL ?? "(place info here)",
    ebay: process.env.NEXT_PUBLIC_EBAY_URL ?? "(place info here)",
  },
  coordinates: {
    lat: process.env.NEXT_PUBLIC_LAT
      ? Number(process.env.NEXT_PUBLIC_LAT)
      : undefined,
    lng: process.env.NEXT_PUBLIC_LNG
      ? Number(process.env.NEXT_PUBLIC_LNG)
      : undefined,
  },
  images: {
    og: "/og-default.svg",
